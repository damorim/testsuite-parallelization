...
def parse_xml_reports(xml_files):
    summary = Counter(tests=0, time=0, failures=0, suites=len(xml_files), classname="")
    for f in xml_files:
...
def parse_xml_reports_classname_error(xml_files):
    date_classname = []
    str_class_name = []
    for f in xml_files:
        if os.path.basename(f) == "TEST-results.xml":
        ...
        try:
            ...
            if failures_attr > 0:
                str_class_name.append(root.get("name"))
    ...
    date_classname.append({"classname": ",".join(str_class_name)})
    
    return date_classname
...
def extract_data_from(project_path):
    ...
    for subdir in os.listdir(project_path):
        ...
        suite_classname = parse_xml_reports_classname_error(xml_files)
        ...
        log_data.append({"name": project_name,
                         ...
                         "path_classname_error": suite_classname[0]["classname"]})
    return log_data
...
if __name__ == "__main__":
    ...
    HEADER = ["name", "exec_id", "compiled", "tested", "mvn_test_time",
              "xml_time", "tests", "suites", "failures","path_classname_error"]

    with open("dataset-rawdata-mode.csv", "w", newline="") as out:
