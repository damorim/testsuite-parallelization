...
def fetcher(query_fields, output_dir):
    ...
    output_is_gradle = CSVOutput("is-gradle-projects.csv", sorted(output_header))
            ...
            for entry in entries:
                ...
                if os.path.exists(project_path) and not has_maven_support:
                    ...
                    if os.path.exists(os.path.join(project_path, "build.gradle")):
                        print("Gradle File found out!")
                        total_is_gradle += 1
                        output_is_gradle.write(entry)
                else:
                    total_is_mvn += 1
                    output_is_maven.write(entry)

                with open('analysis-results.csv', 'w', newline='') as fout:
                    csvout = csv.DictWriter(fout, fieldnames=["total_subjects", "total_is_maven", "total_not_maven","total_gradle"])
                    csvout.writeheader()
                    csvout.writerows([{'total_subjects': total_entries,'total_is_maven': total_is_mvn,'total_not_maven': total_is_not_mvn,'total_gradle': total_is_gradle}])
