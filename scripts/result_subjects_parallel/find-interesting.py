...
class Filter:
    ...
    def __init__(self, params, fields):
        self.__url_template = "https://api.github.com/search/code?q={query}+in:pom.xml+filename:pom.xml+repo:{repo}"
    ...
    def __repr__(self):
        return self.__url_template.format(query=self.__params, repo=self.__fields)
...
def checkIfInteresting(full_name):
    ...
    for item in params_parallelism:
        ...
        if not data_result["items"]:
            ...
            items_counter += 1
            #output_parallel.write([{'full_name': full_name,'type_params_parallel': item}])
    ...
    # for item in params_parallelism:
    #     find_interesting_logger.warning("Checking pom.xml with params: {}".format(item))
    #     filter_obj = Filter(item,full_name)
    #     data_result = filter_obj.fetch()

    #     if not data_result["items"]:
    #         find_interesting_logger.warning("Not Found params: {}".format(item))
    #     else:
    #         find_interesting_logger.warning("Found Out params: {}".format(item))
    #         items_counter += 1
    #         with open('interesting_params_parallelism.csv', 'a', newline='') as fout:
    #             csvout = csv.DictWriter(fout, fieldnames=params_header)
    #             csvout.writeheader()
    #             csvout.writerows([{'full_name': full_name,'type_params_parallel': item}])
...
if __name__ == "__main__":
    ...
    # checkIfInteresting("vavr-io/vavr",output_parallel)
