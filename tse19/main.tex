...
approach to address this problem. \Mar{Sotero, vc. atualizou estas
  macros?} \textcolor{red}{[S.J: atualizado.]} Considering a set of \numSubjs{} Java projects we analyzed
...
parallelization to understand the reasons for not using parallelization.\textcolor{red}{[S.J: nao fizemos isso.]}
...
\Mar{Sotero, por favor, verifique que os numeros que estao aparecendo
  no texto (usados nas macros) estao corretos! Vc. nao precisa
  atualizar o ingles, mas se os dados nao estiverem corretos e vc. nao
  explicar as mudanca (em portugues mesmo) nao vamos ter condicoes de
  revisar.}
\textcolor{red}{[S.J: Ok.]}
We used \github{}'s search API~\cite{githubsearch} to identify
projects that satisfy the following criteria:\Mar{alguma restricao
  sobre o numero minimo de testes por projeto?$\rightarrow$}  \textcolor{red}{[S.J: Nao, mantemos as mesmas restricoes por ordem descendentes das stars]} (1) the primary language
...
file contains the string \emph{mvn}\Mar{ver comentario abaixo}.  We focused on Java for its
...
\Mar{Sotero, vc. usou ou nao usou este recurso da API do GitHub? Estou muito cofuso
  aqui (por isto tive que interromper revisao). Se vc usou, este
  quarto criterio precisa sair e entrar outro dizendo
  explicitamente que usamos isto. Fora isto, logo abaixo vc. diz que
  de um total de 4111, 3154 *nao* eram Maven. Mas, como eh possivel,
  se vc. garante (atraves deste recurso) a presenca de um arquivo pom.xml?}

\textcolor{red}{[S.J: Tivemos dois momentos, o primeiro momento, usamos os mesmos criterios e script do paper ase17 para minerar os subjects. No segundo momento, usamos uma library python chamado PyGithub, ``https://github.com/PyGithub/PyGithub``, para gerenciar recursos do Github API. Utilizamos a funcao search\_code, que busca por especifico arquivo em repositorios ``pygithub.search\_code(pom in:pom.xml filename:pom.xml,order=desc)``, curl Github API ``https://api.github.com/search/code?q=pom in:pom.xml+filename:pom.xml``, e em seguida filtramos esses subjects com os crietrios (2) e (3) do paper.    
]}
...
\end{lstlisting}
  ...
  stars, (3) has been updated on January 1st, 2016 (or later), (4) \Fix{contains
  the string \emph{mvn} in the \emph{readme} file.}\Mar{Please revise.}}
  \vspace{-3mm}
\end{figure}
...
executions.
As of March 5th 2019, our search criteria returned a total of
\SubjectsGithub{}
subjects.
...
\begin{figure}[ht]
  ...
  \numSubjs{} projects to conduct our study.\Mar{Sotero, este plot e
    macros estao atualizados?}} \textcolor{red}{[S.J:Atualizado.]}
\end{figure}

\label{sec:setup}

\Mar{Por que isto esta em uma secao ``Objects of Analysis''? Eh esta
  mesmo a configuracao de maquina que vc. usou.} \textcolor{red}{[S.J:Atualizado.]}
To run our experiments, we used a Intel Xeon ES-2650 v4 (2.20 GHz)
