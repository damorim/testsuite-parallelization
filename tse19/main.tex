...
\author{Sotero Junior, Jeanderson Candido, Marcio Ribeiro and~Marcelo d'Amorim
        
% <-this % stops a space
\IEEEcompsocitemizethanks{
\IEEEcompsocthanksitem 
S. Junior and M. d'Amorim are with the Universidade Federal de
Pernambuco, Recife, PE, Brazil.
E-mail: \{srsj2, damorim\}@cin.ufpe.br

\IEEEcompsocthanksitem 
J. Candido is with the Delft University of Technology, The Netherlands.
E-mail: jeandersonbc@gmail.com

\IEEEcompsocthanksitem 
M. Ribeiro is with the Computing Institute, Federal University of Alagoas, Maceio, AL 57072-900, Brazil.
...
\begin{abstract}
Dealing with high costs of testing remains an important problem in
Software Engineering.  Test suite parallelization is an important
approach to address this problem. \Mar{Sotero, vc. atualizou estas
  macros?} \textcolor{red}{[Sotero: atualizado.]} Considering a set of \numSubjs{} Java projects we analyzed
from \github\, we found that \percentMedLongRunning{} of the them
contain long-running test suites, but parallelization features still
seem underutilized in practice~---~only \percentParallelUpdated{} of
projects with long-running test suites parallelize test
execution. According to a survey we conducted, the main reason for
adoption resistance was the concern to deal with concurrency issues
that could result in test flakiness.
%% This Results suggest that, on
%% average, developers prefer high predictability than high performance
%% in running tests.

\Fix{revise...}
This paper reports our findings on
...
tool developers to speed up test execution. A tool is available to \Fix{...}.
...
\IEEEPARstart{D}{ealing} with high testing costs has been an important
...
%Distributing testing workloads in-house is also a
%viable solution at smaller-size organizations. 
...
use of commodity hardware\Comment{ (\eg{}, existing workstations)} is
...
and continuous
integration~\cite{Saff:2003:RWD:951952.952340}.\Mar{revisit this later
  on...} In addition, we propose a solution to unleash the power of
test parallelization reducing (\Fix{eliminating?}) test flakiness due
to race conditions.
...
\Mar{We need to reorganize this a bit as to accomodate the new
  addition to the paper...Revisit this later...}
...
often share data\Fix{cite Mike's paper, bell-etal-esecfse2015}. Data
...
tests when dependent methods exist across different classes\Fix{cite
  Mike's work}. For the cases the developer prepared code against that
...
(\CodeIn{parallel} parameter) within each forked JVM. In addition, Maven reuses
created JVMs on subsequent forks when execution of a test class
terminates (\CodeIn{reuseFork} parameter). This is an example of a
...
We used the Python library PyGitHub~\Fix{cite}, which uses the
...
directory.\Comment{ A Maven project may contain several sub-modules
  with multiple \pomf{} files.}  Then, considering this set of Maven
...
because of environment incompatibility (\eg, missing\Comment{ required
  web browser or database management system}~DBMS) and
...
detect and mitigate flakiness~\Fix{cite cite}, so we considered this
...
\CodeIn{@Ignore} annotation to discard the failing tests as we did not
want a premature termination of a test run to influence in time measurements.  Our final set of
subjects include  \numSubjs{} projects. 

 %% \textcolor{red}{[S.J: Foi possivel observar que houve uma consideravel conversao de ferramenta de automacao, do Maven para Gradle,  dos 3154 subject que nao eram Maven, 2302 eram Gralde (73\%). Projeto como o MapDB, que e bastante citado no paper de ase17, atualmente converteu-se a Gradle. ]}
...
of analysis we presented in Section~\ref{sec:introduction}.
...
\newcommand{\numRQIssuesTwo}{RQ7}
\newcommand{\RQIssuesTwo}{How effective is selective re-execution in mitigating flakiness??}
...
over 77.53\% of the cases.\Comment{  Such scenarios constitute the majority of the
...
execution.}  For instance, the project \CodeIn{springboot-security-jwt}
...
\CodeIn{T-SNE-Java}, a project from the \longg{} group.
...
\CodeIn{\Comment{ Apache} Hadoop} (using
configuration \ForkSeq), \CodeIn{\Comment{ Apache} Hadoop-common} (using configuration
\ForkSeq{}), \CodeIn{\Comment{ Apache} Tajo} (using configuration \ForkSeq), \CodeIn{\Comment{ Apache} Hawkbit} (using configuration \ForkSeq), \CodeIn{\Comment{ kiegroup} Optaplanner} (using configuration \ForkSeq), \CodeIn{\Comment{ lejon} T-SNE-Java} (using configuration \ForkSeq), \CodeIn{Kubernetes Java Client} (using configuration \SeqClassParMeth)   and
\CodeIn{Netflix Curator} (using configuration \ForkSeq).
Curiously, we also found that project \CodeIn{Alibaba Nacos}, \CodeIn{Rultor},
and \CodeIn{Jcabi} were not detected using this methodology.
...
Three cases worth special attention: \CodeIn{Togglz}, \CodeIn{Chaos} and \CodeIn{Cache2k}.
We note that parallel execution in \CodeIn{Togglz} was
...
sequentially by default. This was also observed in the projects \CodeIn{Zxing}, \CodeIn{Optaplanner}, \CodeIn{OpenNLP} and \CodeIn{Leshan}.
...
configuration.
Finally, the speedup observed in projects \CodeIn{Cache2k}  was
...
\midrule%
Long & \Comment{ Apache } Avro & 14.91m & 13.50m & 1.10x\\%
Medium & \Comment{ Cache2k } Cache2k & 4.58m & 2.24m & \cellcolor{lightgray}2.04x\\%
Medium & \Comment{ spring-data-cassandra } Cassandra & 4.43m & 4.02m & 1.10x\\%
Medium & \Comment{ Bounces } Chaos & 1.64m & 1.54m & \cellcolor{lightgray}1.06x\\%
Long & \Comment{ Javaparse } Javaparser & 7.04m & 6.16m & 1.14x\\%
Medium & \Comment{ Jcabi Github } Jcabi & 2.33m & 1.31m & 1.77x\\%
Medium & \Comment{ Facebook } Jcommon & 1.59m & 1.44m & 1.10x\\%
Medium & \Comment{ Eclipse } Leshan & 1.66m & 1.53m & 1.08x\\%
Medium & \Comment{ Atmosphere } Nettosphere & 2.40m & 2.07m & 1.16x\\%
Medium & \Comment{ Apache } OpenNLP & 3.52m & 3.24m & 1.08x\\%
Long & \Comment{ Kiegroup } Optaplanner & 10.79m & 10.06m & 1.07x\\%
Long & \Comment{ Floraguncom } Search Guard & 13.24m & 6.53m & \cellcolor{lightgray} 2.02x\\%
Medium & \Comment{ Togglz } Togglz & 4.11m & 4.04m & \cellcolor{lightgray}1.01x\\%
Long & \Comment{ Twilio } Twilio Java & 12.57m & 11.38m & 1.10x\\%
Long & \Comment{ Vavr io } Vavr & 5.73m & 5.09m & 1.12x\\%
Medium & \Comment{ Zxing } Zxing & 4.61m & 4.28m & 1.07x\\%
...
executions}, we considered twelve reruns reasonable for this
experiment.
...
95.8\% of the failures were caused by a \CodeInF{null} dereference and 2.7\% were
...
(1.5\% of the total): \CodeInF{EOFException} (0.2\%),
\CodeInF{FileSystemAlreadyExistsException} (0.5\%), and
\CodeInF{BufferOverflowException} (0.8\%). Results suggest that
...
in all configurations were \CodeIn{Activity}, \CodeIn{Incubator Dubbo}, \CodeIn{DROID}, and
\CodeIn{Hawkbit}.  It is worth highlighting the unfortunate case of
\CodeIn{Hawkbit}, which manifested more than 9\% flaky tests in
...
0\% of flakiness have been reported in 47 of the 105 (=5x21)
...
\input{mitigating-flakiness-speedup-table}
...
\item \emph{Incentivize forking.}~Forked JVMs manifest low rates of
  ...
  \CodeIn{Hawkbit}, projects manifest flaky tests in low rates 0.23\%
