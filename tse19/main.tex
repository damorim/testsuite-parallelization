...
projects use Gradle. Although parallelization can
be used in Gradle, we decided to keep using using Maven for
...
bias. For projects with less failing tests, we used the JUnit's
\CodeIn{@Ignore} annotation to discard the failing tests as we did not
want a premature termination of test run to influence in timings.  Our final set of
subjects include  \numSubjs{} projects. 
...
execution.}  \textcolor{red}{[S.J: @Marcio existe alguma caracteristica comum, para que eu possa identificar atraves de script no pom.xml raiz, quais projetos tem testes habilitados por custom params? @Marcelo @Marcio voces acham interessante colocar esse dado aqui ? quantos projetos possuem essa caracteristica?   ]}For instance, the project \CodeIn{springboot-security-jwt}
...
individual test cases per project.\textcolor{red}{[S.J nao teremos mais esse plot.]}
