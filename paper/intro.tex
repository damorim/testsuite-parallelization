...
dimension measures to which extent open-source projects contain costly
test suites.  In the limit, parallelization would be fruitless if all
projects contained short-running test suites. The second dimension
measures how often open-source projects, irrespective of their
associated testing costs, use parallelization schemes.  The third
dimension evaluates how developers involved in costly projects not
using test suite parallelization perceive this technology.  The fourth
dimension evaluates the observed impact of parallelization (when
actually used) on cost.  Finally, the fifth dimension evaluates the
issues and benefits associated with running test suites with different
parallel configurations (/modes).  Overall, this study help us layout
the current state-of-the-practice about usage and impact of test suite
parallelization in open-source projects.
