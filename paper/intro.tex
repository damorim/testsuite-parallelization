...
large IT organizations, with massive code and test bases.  Evidence on
the criticality of running the build and testing processes efficiently
include the Google's TAP system~\cite{google-tap,google-ci} and
Microsoft's CloudBuild system~\cite{prasad-shulte-ieee-microsoft-ci}.
Note, however, that the regression testing problem is not specific to
giants from the IT sector.  For example, a recent paper from the
...
selection~\cite{yoo-harman-stvr2012}.  These approaches reduce cost by
reducing the amount of tests that require execution.  It is important
to bring to light, however, that the complementary approach of
balancing the test workload, either within or across machines, is
sound (as selection techniques such as
Ekstazi~\cite{gligoric-etal-issta2015}) and very popular in industry.
For example, as of August 2013, the Groupon PWA system, which powers
...
private server farm for increased security is a legitimate approach to
mitigate the regression testing problem at large organizations.
However, for projects with low budgets yet moderate or heavy testing
workloads, this solution may not be economically viable.\Mar{can you
  please estimate cost?  what about free services?  what about
  jenkins?}
...
The amount of serious open-source project under development today is
impressive.\Mar{give example, provide stats.}  This paper focuses on
optimizing regression testing of open-source projects using commodity
hardware.  This is an important problem assuming that it is not rare
for open-source projects to face high testing costs.  \Fix{mention
  similar but different work} To the best of our knowledge no prior
