...
\textbf{potential} measures the potential of parallelization to reduce
testing costs.  In the limit, parallelization would be fruitless if
all projects used short-running test suites or if the bulk of
execution cost was dominated by a relatively small number of tests in
the suite.  The dimension \textbf{effectiveness} measures the observed
impact of parallelization\Comment{~---~when used in selected
  open-source projects~---~} on testing costs.  Intuitively, benefits
are proportional to the cost reduction that parallelization brings.
In the limit, the overhead of spawning virtual machines or threads can
...
\textbf{tradeoff} evaluates the issues and benefits associated
with\Comment{running test suites with} different parallel
configurations.  The configuration choice can affect both test cost
and test flakiness.  Flakiness is caused by the non-determinism
resulted from running tests in different
schedulings. Paretto-efficient~\cite{hwang-masud-1979} configurations
can guide future developments in this area.
...
%%% TeX-master: "main"
%%% End:
%%  LocalWords:  jenkins API VMs datarace
