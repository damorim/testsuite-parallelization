...
server farm (for increased security) to automate the testing process
may be a legitimate approach to mitigate the regression testing
problem at large organizations.  However, building such infrastructure
may not be feasibile for open-source and low-budget projects.  For
...
each~\cite{kim-etal-fse2013}.  This paper assumes that, given that it
is not rare for open-source projects to face high testing costs, it is
important to run test suites efficiently using commodity hardware.
