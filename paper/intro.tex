...
of test failures.  Dealing with high testing costs is not uncommon at
large IT organizations.  The Google TAP
system~\cite{google-tap,google-ci} and the Microsoft CloudBuild
system~\cite{prasad-shulte-ieee-microsoft-ci} show that efficiently
building and testing the software are a critically important process
at Google and Microsoft.  The regression testing problem is, however,
not specific to industrial giants.  For example, a recent paper from
the automotive industry reported that test suites can take days to
...
computers with 24 cores each~\cite{kim-etal-fse2013}.  For open-source
projects and lower-budget projects, it is still important to run test
suites efficiently in commodity hardware.
