...
\Jbc{
In practice, some test suites may not be prepared for a multithreaded
execution.
For instance, developers may unconsciously create tests assuming an
specific order of execution.
While parallelism may speed up tests, in those cases,
tests may yield non-deterministic results due to data races.
In general, a test that either pass or fail under the same conditions
is known as a ``flaky test''~\cite{luo-etal-fse2014}. It is important
to measure the presence of test flakiness and the trade-offs to
overcome this issue.}
