...
usage and impact of low-level parallelization to speed up testing
...
integration~\cite{Saff:2003:RWD:951952.952340}.
...
technology.  It is important to measure resistance of practicioners to
...
\emph{issues} reports the observed problems associated with running
tests in parallel.  For example, test flakiness, which is caused by
interference on shared state from non-deterministic executions of test
methods and classes.  We briefly summarize our findings in
the following.
...
\vspace{1ex}\noindent\textbf{Feasibility.}~To assess how prevalent
...
using the technology.\Comment{ For the quantitative analysis we
  ...
  parallelization.}  Considering only those projects that take longer
than a minute to run their test suites, we found that only
...
\vspace{1ex}\noindent\textbf{Speedups.}~We used two setups to measure
execution speedups.  In one setup we measured the speedups obtained on
projects that we knew a priori that ran their test suite in parallel
by default.  We did not change parallel configurations on that setup.
For comparison, we disabled parallel execution to obtain the running
time of a sequential execution.  In the other setup, we evaluated how
execution scales with the number of available cores in the machine.
\Mar{need to repeat those!}\Fix{Considering the first setup, results
  indicate that the average speedup of parallelization was
  \avgSpeedup{}x.  Although we found cases with very high speedups
  (\eg{}, 9.2x), we found some cases where speedups were
  insignificant.}
