...
For tests that change the state of a shared resources (\eg, file or
in-memory collection), data races are likely to occurr. 

Data races are likely to occurr in a multithread execution as 
different threare favors data races when Some tests may produce non-deterministic
outcomes in a multithreaded ex when
executed in 
including test flakiness~\cite{luo-etal-fse2014}. A test case with
non-deterministic outcome is also known as ``flaky test''.  are
known as test with non-deterministic outcome and
}
