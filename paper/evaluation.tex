...
has at least 100 stars; (3) latest update on (or after) March 1st,
2017; (4) the \emph{readme} file contains the string \emph{mvn}.
...
\end{lstlisting}
    ...
    stars, (3) updated on March 1st, 2017 (or later), (4) contains
...
root directory.  As of March 24th 2017, our search criteria returned
\Fix{615} subjects. From the \Fix{615} downloaded projects, \Fix{46}
were not Maven projects (or did not have a \pomf{} in the root
directory) and 159 were in a unstable revision.  From these 159
unstable subjects, \Jbc{elaborate what is unstable (\eg, (1) how many
could not resolve dependencies? (2) how many could not compile sources
and tests? (3) could not run tests due to incompatible testing
environment?}. Our final set consists in \numSubjs{} testable
subjects.
...
Figure~\ref{fig:rq1-barplot} represents a lower bound estimation for
time cost because some projects omit long-running tests on their
default execution. For instance, it is necessary to invoke Maven with
additional parameters to run integration tests from project
\CodeIn{apache.maven-surefire} (nearly 30 min in our environment).
Also, some tests may finish earlier than expected due to existing bugs
on the ongoing revision.  From the \numSubjs{} compiled projects, 286
successfully executed all tests and 118 reported test failures.
\Jbc{elaborate failed subjects: how much \% of the tests failed?}
...
tests monopolizing end-to-end execution time were rare.
