...
\newcommand{\RQB}{How the time cost from a regression-test is distributed
among the test cases?}
\newcommand{\RQC}{How often parallelization settings appear in
build files?}
...
by the mean. \Jbc{I need to clarify the relationship
"well/bad-balanced" regression test and relative standard deviation}
...
\ref{sec:modes}), we considered the \numMedLong{} subjects from the
\longg{} and \medg{} groups identified in Section~\ref{sec:rqA}. We
...
\Jbc{methodology is poorly described...}
\Fix{
We computed the frequency of the
parallelism modes using a two-steps approach: the first step
(discovery) automatically finds relevant build files (\ie, \pomf{}
files with parallelization settings); the second step (inspection)
consists in a manual inspection to confirm the presence of parallelism
settings in the build file and classify them according to the
parallelism level.  Figure \Fix{removed}
describes the discovery step: we list the paths of all build files and
filter only the files that contain any of the keywords
...
parallel setting \Fix{cite surefire doc}. Our first step is complete
but is not sound since the line that matches one of keywords may not
represent part of a real setting (\eg, comments).  We eliminate
false-positives in the confirmation step.
}

\newcommand{\pomMedLong}{2851}

\Jbc{results from table}From the \numMedLong{} subjects, we found
\pomMedLong{} \pomf{} files and selected the 109 files that matched
with any of the configuration keywords (\ie, \CodeIn{forkCount},
\CodeIn{forkMode}, and \CodeIn{parallel}).  From these 109 build
files, we eliminated six false-positives: two were names of project
modules (\eg, \emph{camel-cookbook-parallel-processing}); one was part
of a comment; three were commented configurations. The 106 build files
with valid configurations are distributed in 41 projects from our
sample.  Table~\ref{tab:inspection-table} summarizes our results.
