...
measure cost we used the following methodology. \Jbc{The following
  steps may change $\rightarrow$} Figure~\ref{fig:mvn-execution} shows
...
SSH. In addition, we configured the \CodeIn{isolcpus} option from the
...
processes.\Mar{can we cite this?}
...
\longg{}-running.
\Luis{rework this... $\rightarrow$}To reduce the number of build files
to inspect, we consider only \pomf{} files that contain the
configuration node inside a \CodeIn{plugin} tag with a child tag named
\CodeIn{artifactId} with the content \CodeIn{maven-surefire-plugin}.
