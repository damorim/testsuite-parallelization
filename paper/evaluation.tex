...
cost is nearly equally distributed. Regression tests that are well
distributed may benefit from parallelism since more tests executes at
the same time while the opposite scenario may require a different
approach. In the later scenario, executing tests in parallel may have
insignificant impact since a small subset of test cases dominates the
execution.

To evaluate the distribution of execution cost among test cases, we
proposed the \emph{balance} statistic to indicate how equally
distributed is the test execution. \Fix{show formula}.  \Fix{explain
formula}

Results indicated that \Fix{...elaborate...}. Figure~\Fix{N}
summarizes the results. \Fix{show a histogram with the frequency of
balance for the subjects analyzed}
