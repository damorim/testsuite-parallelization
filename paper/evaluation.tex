...
with different parallel configurations. \Jbc{I think we must have a
sentence to motivate why we want to understand this relationship}.  We
...
%% that satisfies the following criteria: (1) tests run in sequence by
%% default and (2) test suite consists only by passing tests. The first
%% criteria is to use the default execution (\ie, configuration \Seq{})
%% as a baseline to compare speedup with the other parallel
%% configurations. The second criteria is to avoid introducing noise when
%% measuring the rate of flakiness caused by the non-deterministic
%% execution of tests. We are interested only in failures manifested when
%% parallelism is enabled. Since the number of failing tests may vary, we
%% run each project three times and report the average elapsed time and
%% rate of flakiness. 
...
\multirow{2}{*}{\emph{Subject (module)}} & \multicolumn{2}{c|}{\emph{\Seq}} &
    ...
    & \subcol{}\\%
\midrule%
OpenMRS Core (\CodeIn{api})           & \entry{13.8m}{X} & \entry{-}{58.07\%} & \entry{1.86x}{65.55\%} & \entry{2.01x}{61.56\%} & \entry{1.51x}{0\%} & \colheader{\cellcolor{lightgray}JVM Crash}\\%
Apache Flume (\CodeIn{core}) & \entry{8.3m}{Y} & \entry{2.82x}{18.11\%} & \entry{-}{0\%} & \entry{2.78x}{16.84\%} & \entry{-}{0\%} & \entry{2.87x}{18.88\%}\\%
Apache Giraph (\CodeIn{core})  & \entry{7m}{Z} & \entry{2.08x}{5.08\%} & \colheader{\cellcolor{lightgray}JVM Crash}  & \entry{-}{0\%} & \entry{-}{0}\% & \colheader{\cellcolor{lightgray}JVM Crash}\\%
Struct (\CodeIn{rest})  & \entry{Xm}{Z} & \entry{x}{\%} & \entry{x}{\%} & \entry{x}{\%} & \entry{x}{\%} & \entry{x}{\%}\\%

%% Linkedin Pinot & 3480.091s & 1703.958s & 24.726s & 2229.822s & 783.12s & 761.973s\\%
%% Linkedin Pinot & 58m & 2.04x & 140x & 1.56x & 4.44x & 4.56x\\%
%% Jenkins CI Github Plugin & 3298.603s & 2049.709s & 1547.261s & 1240.544s & 2975.232s & 227.214s \\%
%% Jenkins CI Github Plugin & 55m & 1.60x & 2.13x & 2.65x & 1.10x & 14.51x \\%
%% Kite SDK & - & - & - & - & - & -\\%
%% Apache Giraph & 1041.884s & 37.666s & 116.78s & 1037.105s & 1038.39s & 45.992s\\%
%% Apache Giraph & 17.36m & 27.66x & 8.92x & 1.00x & 1.00x & 22.65x\\%
%% OpenMRS Core & - & - & - & - & - & -\\%
%% Jenkins CI Docker Workflow Plugin & 447.069s & 490.675s & 27.69s & 8.244s & 439.207s & 38.3s\\%
%% Jenkins CI Docker Workflow Plugin & 7.45m & 0.91x & 16.14x & 54.22x & 1.01x & 11.67x\\%
