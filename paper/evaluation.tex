...
\Comment{While there are several causes that may introduce randomness on test
results, our concern is on flakiness caused by
parallel execution.}
...
\Jbc{From the \SubjectsGithubConsistant{} projects with
...
for replication (on our website~\cite{ourwebpage})\Comment{ at \Fix{create gh-pages}}.  This
...
\begin{minipage}{8cm}
  ...
  execution time were rare. Time most often is distribuited evenly
  across test cases.}
\end{minipage}
}
\end{center}
...
\midrule%
Medium & Californium & \Fix{Xm} & \Fix{Xm} & \Fix{Yx}\\%
Medium & \Comment{BounceStorage }Chaos\Comment{ HTTP Proxy} & 1.51m & 1.47m & \cellcolor{lightgray}1.01x\\%
...
Medium &Jcabi\Comment{ GitHub} & 2.76m & 0.30m & \cellcolor{lightgray}9.2x\\%
Long &\Comment{ Hazelcast }Jet& 8.26m & 3.67m & 2.25x\\%
Long & \Comment{apache} Mahout & \Fix{Xm} & \Fix{Xm} & \Fix{Yx}\\%
Long &\Comment{ Jankotek }MapDB& 10.06m & 8.58m & 1.17x\\%
Medium & \Comment{apache} OpenNLP & \Fix{Xm} & \Fix{Xm} & \Fix{Yx}\\%
Medium & \Comment{yegor256} Rultor & \Fix{Xm} & \Fix{Xm} & \Fix{Yx}\\%
Medium & \Comment{yegor256} Takes & \Fix{Xm} & \Fix{Xm} & \Fix{Yx}\\%
Long & \Comment{vavr-io} Vavr & \Fix{Xm} & \Fix{Xm} & \Fix{Yx}\\%
\midrule
...
execution in \CodeIn{Log4J2} was ineffective.  We found that Maven
...
Maven to run \Comment{configurations \emph{C*}}test suites with
...
with the command \CodeIn{mvn dependency:build-classpath}. Once we have
...
\CodeIn{timeout} command~\cite{timeout-cmd} configured to dispatch a
...
%%  LocalWords:  JCTools RipMe classpath ParallelComputer livelock
%%  LocalWords:  mapdb workflow XPath
