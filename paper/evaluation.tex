...
\multirow{2}{*}{\emph{Subject}} & \multicolumn{2}{c|}{\emph{\Seq}} &
    ...
    & \subcol{}\\%
\midrule%
AWS SDK Java (\CodeIn{core})  & \entry{3.7m}{847}  & \entry{1.95x}{2.24\%} & \entry{2.47x}{2.77\%} & \entry{3.70x}{4.01\%} & \entry{1.85x}{} & \entry{3.36x}{}\\%
...
the six parallel configurations from Section~\ref{sec:modes}, and we
randomly selected 10 subjects from the medium and long groups (\ie,
tests suites that ran in at least one minute) identified in
\numRQFeasibilityOne{}. \Jbc{Is this legit/acceptable/convincing?
$\rightarrow$}The rationale for this criteria was to maximize the
diversity of domains from the subjects, assuming these subjects have
test suites with different characteristics (\eg, architectural design
and rate of cpu/io-bound operations).
\Comment{
