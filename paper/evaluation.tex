...
\lstset{
    ...
    morekeywords={o, DskipTests, Dmaven, javadoc, skip},
    deletekeywords={true}
}
\begin{lstlisting}
...
\begin{figure}[h!]
    ...
    \Fix{On the left side, results from sequential execution; on the
    right side, results from default execution.}}
\end{figure}

Considering the groups \emph{medium} and \emph{long} (\Fix{X}
subjects), we investigated the relationship between the number of
tests cases and the time cost from subjects.  Figure~\ref{fig:scatter}
...
\Fix{elaborate: As expected, Figure~\ref{fig:scatters} reveals that
the number of tests cases are not related to the elapsed time.
\Fix{explain briefly what dominates a test execution/types of tests}.
We ignored outliers for better visualization.} Results indicate that
\Fix{...conclusion...}.
