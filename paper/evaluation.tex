...
Figure \Fix{removed} describes the discovery step: we list
the paths of all build files and filter only the files that contain
any of the keywords \CodeIn{forkMode}, \CodeIn{forkCount}, or
\CodeIn{parallel}.  Notice that, for Maven projects, any of these
keywords exist in any possible parallel setting \Fix{cite surefire
doc}. Our first step is complete but is not sound since the line that
...
we confirmed \Fix{M} valid settings from \Fix{X} subjects.
\Jbc{Elaborate results} Table~\ref{tab:inspection-table} summarizes
the results from the file inspection and Figure~\ref{fig:freqmodes}
shows the distribution of parallelism modes from the \Fix{M} settings.
...
\begin{table}[ht!]
    ...
    \begin{tabular*}{0.48\textwidth}{@{\extracolsep{\fill}}cccc}
        ...
        \midrule%
        Long   & 1613 & 79/\Fix{z} & \Fix{X}/\numLong{}\\%
        Medium & 1240 & 30/\Fix{y} & 22/\numMed{}\\%
        \midrule%
        Total  & 2853 & 109/\Fix{z+y} & \Fix{w}/\numMedLong{}\\%
        \bottomrule%
    \end{tabular*}
