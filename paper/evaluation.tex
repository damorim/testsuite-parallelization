...
We tooke the following actions to isolate our environment from
...
\begin{tabular}{lllll}
\toprule
Group & Subject & T$_{\text{p}}$ & T$_{\text{s}}$ & Speedup\\%
\midrule%
Long   & JenkinsCI Gerrit Trigger Plugin & \Fix{?} & \Fix{?} & \Fix{?}\\%
Medium & Apache Logging Log4J2           & \Fix{?} & \Fix{?} & \Fix{?}\\%
Medium & Jankotek MapDB                  & \Fix{?} & \Fix{?} & \Fix{?}\\%
Medium & Javaslang                       & \Fix{?} & \Fix{?} & \Fix{?}\\%
Medium & Jcabi Github                    & \Fix{?} & \Fix{?} & \Fix{?}\\%
\bottomrule%
\end{tabular}}
\caption{\Fix{Comparacao de tempo sequencial e paralelo}}
\label{tab:speedup}
\end{figure}
