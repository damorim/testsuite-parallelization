...
to eliminate false-positives. Given a list of subjects, our script
searches recursively all \emph{pom.xml} files from the project
directory and uses the command-line tool \emph{grep} to highlight the
presence of the keywords \emph{forkMode}, \emph{forkCount}, and
\emph{parallel}. Any of these keywords exist in any possible
combination of parallel settings (see Section~\ref{sec:modes}). Before
the manual inspection, we found that 37.14\% (13 out of 35) and 62.5\%
(35 out of 56) of subjects from the \emph{long} and \emph{medium}
groups did not have any of the keywords. After the manual inspection
\Fix{...elaborate...}

\Fix{We discovered that \Fix{X}\% and \Fix{y}\%
of subjects (\emph{long} and \emph{medium}, respectively) did not have
any evidence of low-level parallelism. It is important to mention that
this result an upper bound limit since the keyword highlighted could
not be part of a real configuration (\eg, part of a comment). To
investigate further the projects reported by our script, we manually
inspected \Fix{X} configuration files to eliminate false-positives.}
\Fix{...elaborate...}
