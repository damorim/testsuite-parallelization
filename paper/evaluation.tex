...
\numSubjsTotal{} projects we analyzed.  A key factor to determine if
low-level parallelism will be effective to speedup test execution is
to identify whether or not a small fraction of tests dominate test
suite execution time.  If that is the case, it is unlikely that test
suite parallelization will help as a test artifact is the smallest
working unit in test frameworks (see Section~\ref{sec:frameworks}).
Ideally, one would hope that time is distributed uniformly across
tests in the suite.
...
To evaluate the distribution of time, we analyzed the execution time
