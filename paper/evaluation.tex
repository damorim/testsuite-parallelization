...
three extreme cases: \CodeIn{Chaos}, \CodeIn{Jcabi} and \CodeIn{Log4J2}.
The first, the default configuration is \textbf{C2}, however, the test is
composed of takes $>$99\% of the test time. In this case, when running
in sequential the execution time is almost the same as the parallel
execution.
...
methods. In this case, a high slowdown is noticed when running those
big methods sequentially.
The third, runs parallel configuration in \Fix{80\%} of the project
modules, however, the test time is dominated by one of the
sequentially running modules.
...
\Fix{x=83-11-y} projects.  From the initial list of \numMedLong{}
project, we discarded 11 projects that we knew a priori used
parallelization, and \Fix{y} projects that we could not find
...
we sent plain-text e-mails to those developers with the survey.  In
...
environment of the developers.  Overall, we found that \Fix{...}
...
To answer \numRQF{}, we considered the top ten projects with
long-running test suites and all parallel modes (see
Section~\ref{sec:modes}). In this experiment, we run the test suites
with different parallel modes and evaluated the average time cost and
rate of failing tests. To avoid noise, we carefully selected only
subjects with nonfailing tests to avoid noise in our measurements.

\Fix{Elaborate results from efficiency}

Figure~\ref{tab:rq6-efficiency} summarizes our findings.
...
\Fix{Elaborate results about flakiness}

Figure~\ref{tab:rq6-flaky} summarizes our findings.
...
\label{seq:rq6-tradeoffs}
\Fix{discuss trade-offs and elaborate conclusion....}
