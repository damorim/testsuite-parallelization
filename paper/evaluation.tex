...
low.\Comment{ Figure~\ref{fig:sd} shows the number of projects within
  specific ranges of $\sigma$ values.}  Note that high standard
...
medium group.  This project contains only \Fix{65} tests, \Fix{62} of
...
as \Fix{$>$99\%} of the execution cost is dominated by only \Fix{3\%}
...
indicates that the project \CodeIn{uber\_chaperone} was an extreme
...
smoothly across \Fix{529} tests, of which \Fix{119 (23\%)} take more
than \Fix{1s} to run.
...
distribution of test suite sizes across groups.\Mar{$\leftarrow$ show
  stats to indicate discrimination of two distributions} Furthermore,
...
We configured Maven to output all configuration variables.  The
rationale is that, for each project's module, we can capture relevant
configurations and classify their parallelization level (see Section
\ref{sec:modes}) based on their values. To avoid waiting all tests to
terminate, we just run the bootstrap phase and we skip the actual test
execution.  According to Maven's
documentation~\cite{maven-surefire-plugin}, any parallelization
setting depends either on (1) \CodeIn{parallel} to define the
parallelism mode within a JVM followed by \CodeIn{threadCount} or (2)
\CodeIn{forkCount} (or \CodeIn{forkMode} in older versions) to define
the number of forked JVMs. For each test execution, we look for theses
parameters and their values to map to one of the possible
parallelization modes.  For instance, if a given project contains a
module with the setting \CodeIn{<forkCount>1C</forkCount>}, the
possible classifications are \ForkSeq{} or \ForkParMeth{} depending on
the presence and the value of the parameter \CodeIn{parallel}.
...
to not inflate our final results. Finally, some projects use a value
informed as an argument instead of adding the value statically in the
configuration value. In this case, we consider all possible values for
the parameterized parameter.  From the \numMedLong{} projects, we
...
\begin{figure}[ht!]
    ...
    \begin{tabular*}{0.48\textwidth}{@{\extracolsep{\fill}}llll}
        ...
        medium & Jcabi & 1 & \ParClassParMeth{}\\%
...
From the valid replies, we confirmed that the projects we sent the
e-mails takes more than 1 minute to run tests and \emailsSeq{}
confirmed that those projects do not use parallel testing using Maven
Surefire. The other projects that we do not confirm, need and extra
parameter to execute in parallel mode \Luis{make sure that they are
inside the static analysis sheets made before}.
...
testing setup.
We also noticed that one project has testing enabled without parallel 
configuration, however, the maintainers never run the test. 
...
another method of parallelization with distributed build machines.
