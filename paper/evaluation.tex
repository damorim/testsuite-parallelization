...
We used the Github's Search API to fetch the top 1,000 Java projects
with at least 100 stars. The number of stars indicates the interest
and appreciation from the community to a given project.
...
projects to conduct our study.\Jbc{Subject selection is currently OK
    but I will use another way (eg Google BigQuery) to have access to
more subjects.} For each project, we detected the build manager system
based on the files located in the root directory from the project in
the following precedence: Maven (\emph{pom.xml)}, Gradle
(\emph{gradlew}), and Ant (\emph{build.xml}). From the 1,000
downloaded projects (composed by \Fix{X} Maven, \Fix{Y} Gradle,
\Fix{W} Ant, and \Fix{W} unknown projects), we selected the 154 Maven
projects that we were able to compile\Jbc{Remember to update those
numbers (download + projects) after updating the subject selection
approach.}. We noticed that we are unable to compile other subjects
because other build systems are commonly related to a particular
platform (\eg, Gradle implied Android), the build process is based on
user-defined tasks (\eg, Ant), or the project has unsolved
dependencies.
...
(64-bit version). Our evaluation scripts depends on \Fix{specify:
unix-based tools} to measure CPU usage and \emph{Python 3.4} (or
later). For replication, we provide all the source artifacts publicly
available at \Fix{create gh-pages}, including supporting scripts (\eg,
