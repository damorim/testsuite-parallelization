...
considered the 321 subjects from Section~\ref{sec:subjects} and
...
To evaluate the  use of low-level parallelism, we considered the
...
(\ie, \emph{pom.xml}) from each subject (\emph{long} and \emph{medium}
groups). Initially, our script finds recursively all \emph{pom.xml}
files from the project directory. For each file, we use the
command-line tool \emph{grep} to highlight the presence of the
keywords \emph{forkMode}, \emph{forkCount}, and \emph{parallel}. Any
of these keywords are present in all possible parallel settings (see
Section~\ref{sec:modes}).  We discovered that \Fix{X}\% and \Fix{y}\%
