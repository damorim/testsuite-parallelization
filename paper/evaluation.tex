...
however, cost is distributed more smoothly across \Fix{98} tests, of
which \Fix{8 (8.1\%)} take more than \Fix{1s} to run with the rest of
...
Tarr (64-bit version). This experiment uses configuration \emph{\ForkSeq{}}
\begin{wrapfigure}{r}[0pt]{0pt}%0.525\linewidth
    \centering
...
single test class, \CodeIn{org.mapdb.WALTruncat}, containing 15 test
cases that take over four minutes to run.
...
\fbox{
  \begin{minipage}{8cm}
    \textit{Answering \numRQSpeedupTwo{}:}~\emph{Results suggest that
      ...
      on the speedup one can get related to how well the test suite is
...
\noindent\textbf{JUnit.} It is worth mentioning that we used JUnit as
opposed to Maven to run configurations \emph{C*}.  After carefully
verifying library versions for compatibility issues and comparing
results with JUnit's we observed that several of Maven's executions
exposed problems (\eg{} timeouts).  These issues are documented in our
website~\cite{ourwebpage} through Docker containers.  To address those
issues we implemented basic JUnit runner reflecting the effects of
parallel configurations from Maven Surefire.

\Mar{estou aqui...}

Our script takes a list
o tests classes to run as an input and outputs the results (\ie,
number of test methods, failures, skips, and elapsed time) for each
parallel configuration. Initially, we use an existing Maven log to
identify the test classes to run, and we use the Maven dependency
plugin~\cite{maven-dep} to build the project's classpath with the
command \CodeIn{mvn dependency:build-classpath}. Once we have the
...
nonsignificant differences (a fraction of seconds in most cases).
\Comment{ To obtain parallel configurations, we implemented a script
  ...
  described in Figure~\ref{fig:mvn-execution} except that...  } Also,
we used the \CodeIn{timeout} command~\cite{timeout-cmd} in our scripts
to monitor the execution, and we configured the command to dispatch a
\emph{kill} signal if the test execution exceeds a given time
limit. We imposed this time constraint to avoid hanging indefinitely
the experiment execution due to some thread contention that may occur
(\eg, deadlock or livelock).  Finally, we saved each execution log and
stack traces generated outputed from JUnit to collect the execution
time, the number of failing tests, and for reference to analyze and
diagnose outliers in our results.
