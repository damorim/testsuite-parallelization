...
methodology we used to find these subjects and
...
\midrule%
Medium & BounceStorage \textbf{Chaos} HTTP Proxy  & 1.51m & 1.47m & 
    \cellcolor{lightgray}1.02x\\%
Medium & Apache \textbf{Flink} & 11.79m & 2.57m & 4.59x\\%
Long & Jenkins CI \textbf{Gerrit} Plugin & 51.19m & 40.31m &  1.26x\\%
Medium & Spotify \textbf{Helios} & 4.46m & 1.63m & 2.73x\\%  
Medium & \textbf{Javaslang} & 2.18m & 1.82m & 1.19x\\%
Medium & \textbf{Jcabi} GitHub & 2.76m & 0.30m &
    \cellcolor{lightgray}9.2x\\%
Medium & Hazelcast \textbf{Jet} & 8.26m & 3.67m & 2.25x\\%
Medium & Apache \textbf{Log4J2} & 8.24m & 8.21m & 
    \cellcolor{lightgray}1.00x\\%
Medium & Jankotek \textbf{MapDB} & 10.06m & 8.58m & 1.17x\\%
\midrule
...
\fbox{
  \begin{minipage}{8cm}
    \textit{Answering \numRQD{}:}~\emph{ The average speedup observed
      with default configurations of parallelization was
      \avgSpeedup{}x.}
  \end{minipage}
}
\end{center}
...
\begin{enumerate}
\item How long does it take for test to run in your environment? Can
  you briefly define your setup?
\item Do you confirm that your project does not run in parallel?
...
these participants reporting that test suites were modularized and
those modules are tested independetly in CI servers through different
parameters. A total of \emailsLocal{} participants answered that they
do run tests in their local machines.  Note, however, that CI does not
preclude low-level parallelization.  For example, installations of
open-source CI tools (\eg{}, Jenkins~\cite{jenkins}) in dedicated
servers will still benefit from running tests faster through low-level
test suite parallelization.

% \emailsNotDescribed{} developers did not described their environment.

Considering question 2, participants responded that
\emailsSequential{} of the \emailsProjectsAnswered{} projects did not
execute tests in parallel.  This mismatch is justified by cases where
neither of our checks (static or dynamic) could detect presence of
parallelism.  

We investigated this six projects further, and one of them contained a
false-negative result from \numRQC{}, which means that a configuration file
is compacted inside a Java container. One of them considered a
distributed CI server as parallelism. Two of those projects had a
custom JUnit test runner, which handles the parallelism itself. The
last two projects run the parallelism inside the test method, one runs
a manually created Thread to run some tests, and the other runs in
parallel by using Java 8 collection streams, that allows the
developers to iterate over a list in parallel.
...
\begin{wrapfigure}{r}[0pt]{0pt}%0.525\linewidth
    ...
    \caption{\label{fig:rq5-answers}Summary of developer's answers to
      question~\ref{questionThree}.}
\end{wrapfigure}
Considering question 3, \emailsA{} of developers who answered the
survery did not know that parallelism was available in Maven,
\emailsB{} of developers mentioned that they did not use parallelism
because of concerns with concurrency issues, \emailsD{} of developers
uses a continuous integretion server for run tests while working
\Luis{here I want to say that they use it for something like
  "non-blocking testing" while developing in a local machine},
\emailsD{} of developers thinks that it is not worthy the effort, and
\emailsNA{} of developers did not answered the last question, or did
not knew an answer.
...
Hence, flakiness and speedup are contradictory forces that drive
configuration selection.  Note also that Section~\ref{sec:rqD}
evaluated variable speedup in isolation, focused on project
configuration where parallelism is used by default.

\begin{itemize}
    \item \numRQF{}. \textbf{\RQF{}}
\end{itemize}

To answer this research question we selected projects as follows.  To
avoid noise in measuring flakiness we first discarded projects
containing failing tests.  Then, we sorted projects in descending
order of their execution times and selected the top ten projects from
the resulting set.  In this experiment, the baseline technique is
sequential execution (\ie{}, configuration \Seq{}) and the evaluation
techniques consist of all configuration modes listed in
Section~\ref{sec:modes}.  We ran each of those techniques for \Fix{X}
times computing elapsed time and number of failing tests in each run.
Figures~\ref{tab:rq6-efficiency} and~\ref{tab:rq6-flaky} summarize the
results we obtained.
