...
those modules were tested independetly in CI servers through different
parameters. A total of \emailsLocal{} participants answered that they
do run tests in their local machines.  Note that CI does not preclude
low-level parallelization.  For example, installations of open-source
CI tools (\eg{}, Jenkins~\cite{jenkins}) in dedicated servers will
still benefit from running tests faster, using test suite
parallelization.

% \emailsNotDescribed{} developers did not described their environment.

Considering question 2, we found that \emailsSequential{} of the
\emailsProjectsAnswered{} projects indeed did not execute tests in
parallel.  The mismatch is justified by cases where neither of our
checks (static or dynamic) could detect presence of parallelism.  \Fix{elaborate}
\Luis{i looked at those 6 weird projects, and 1 of them contains a
false-negative from rq4, one of them considered the distributed CI
server as parallelism, 2 of them had a custom test runner, and 2 of
them implements parallelism inside the test (they test two+ things in
one test method.}
