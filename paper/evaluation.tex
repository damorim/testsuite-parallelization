...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.25\textwidth]{plots/subjects/subjects.pdf}
...
To run our experiments, we used a dedicated server with eight cores
and 16 GB of memory, running Ubuntu 14.04 LTS Trusty Tahr (64-bit
version).  Software settings include \Comment{the Linux \emph{sysstat}
package to measure performance, }git to fetch subjects, Java 8 and
Maven 3.3.9 to build and test subjects. Our evaluation scripts depends
on Python 3.4 and Bash to execute and R to analyze the data.  For
replication, all source artifacts are publicly available at
...
documentation\Fix{cite surefire doc}, the set of parameters to run
tests in parallel is described as follows: \CodeIn{parallel} defines
the parallelism granularity within a JVM (\eg, methods or classes);
\CodeIn{threadCounter} defines the size of the thread pool; and
\CodeIn{forkCount} defines the number of forked JVMs. Additional
...
and we skip the actual test execution. \Jbc{Missing: explain how we
mapped the configurations to parallel level.}
...
execution. \Jbc{Remember to explain that we need the previous analysis
because some configurations are unreachable statically when inherited
from some external module}.

%% To answer \numRQC{} we initially mined build files (\ie, \pomf{}) .
%% must contain one of these keywords that
%% contain parallelization keywords, namely \CodeIn{forkMode},
%% \CodeIn{forkCount}, and \CodeIn{parallel}.
