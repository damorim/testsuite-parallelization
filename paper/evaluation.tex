...
In addition, we identified \SubjectsGithubFlaky{} projects with flaky
tests.
As some of our experiments consist in running tests in a multithreaded
configuration, we ignored these projects as it would be impractical
for us to identify whether a test failed due to a race condition.
...
\Jbc{This is not true for projects running tests in parallel by defaut.
Recall from RQ3 that some projects run tests in parallel by default.
