...
in those replies. \Luis{In one specific case, one developer worked in
  multiple projects, and we consider it as a different answer.}
...
sequentially by default. This was also the case of the highlighted
...
test flakiness.  We used the sequential execution associated with each
project as the comparison baseline in this experiment.  We sorted the
subjects by decreasing order of execution time and selected the first
15 projects that uses JUnit 4.7 or later.  The rationale for this
...
website~\cite{ourwebpage} and can be reproduced with our scripts.
...
with the command \CodeIn{mvn dependency:build-classpath}.
Once we have the classpath and the tests to run, we invoke our JUnit
custom runner to run the test classes with different parallel
configurations. For the sequential execution, we invoke the
default JUnit test runner.
For parallelism within a JVM (\ie, the configurations
\SeqClassParMeth{}, \ParClassSeqMeth{}, and \ParClassParMeth{}), we
configured the JUnit runner with
ParallelComputer~\cite{junit-parallel}.
For the configurations with forked JVMs (\ie, \ForkSeq{} and
\ForkParMeth{}), we spawn a JVM with our JUnit custom runner to
execute a test class in an individual process. We configured this
experiment to run at most three JVMs in parallel.
...
processes.  To ensure that our experiments terminate (recall that
deadlock or livelock could occur) we used the \CodeIn{timeout}
