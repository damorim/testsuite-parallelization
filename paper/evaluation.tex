...
identifying time-consuming test suites \Jbc{do we have a better
argument than this?}for further investigation. The
...
that we were able to run the tests. For each subject, we first
compiled the source and later, tested and collected the elapsed time
reported from the build system using regular expressions. To avoid
inflating the reported time, we run the tests ignoring non-related
tasks (\eg, \emph{javadoc} generation). In case of \emph{Maven}
projects, use \Jbc{How to show the following command?} \Fix{mvn test
-Dmaven.javadoc.skip=true}; in contrast, for \emph{Ant} projects, we
used \Fix{ant test} and manually inspected the test execution to
ignore any non-related tasks, if any.
...
were tested in a potentially unstable revision. \Fix{Results indicate
that a relatively high number of projects...}.
