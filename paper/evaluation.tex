...
directory) and \SubjectsGithubNotTestable{} were in an untestable
revision (\eg, missing dependencies or incompatible testing
environment). To ensure that our sample set is stable, we retested the
remaining subjects to eliminate flaky tests (total of
...
\begin{figure}[ht]
    ...
    \caption{\label{fig:subjects}\Fix{this is mocked data!}}
\end{figure}
\vspace{-2ex}
...
and compared the elapsed time to run their tests. \Jbc{The following
steps may change $\rightarrow$}
...
\Jbc{these results will be updated $\rightarrow$} It is important to
mention that Figure~\ref{fig:rq1-barplot} represents a lower bound
estimation for time cost because some tests may finish earlier than
expected due to existing bugs on the ongoing revision.  From the
\numSubjs{} compiled projects, \numSubjsPass{} successfully executed
all tests and \numSubjsFail{} reported test failures.  Also, some
...
(nearly 30 min in our environment).
...
According to Maven's documentation \Fix{cite surefire doc}, any
parallelization setting depends either on (1) \CodeIn{parallel} to
define the parallelism level within a JVM followed by
\CodeIn{threadCounter} or (2) \CodeIn{forkCount} (or \CodeIn{forkMode}
...
keywords, namely \CodeIn{forkMode}, \CodeIn{forkCount}, and
\CodeIn{parallel}.  This mining step, however, is not sound as a
...
Figure~\ref{fig:freqmodes-static} summarizes our findings. \Jbc{Given that we
...
\fbox{
  \begin{minipage}{8cm}
      ...
      that \Jbc{summarize results from both parts (static and
      dynamic)}.} 
  \end{minipage}
}
\end{center}
...
To enforce sequential execution, \Jbc{explain methodology}.
\Luis{
Primeiro nos avaliamos os \pomf{} de alguns projetos long que 
rodavam em paralelo.
Constatamos que a maioria dos projetos possuiam um no configurando o 
plugin do maven surefire, e alguns projetos ``herdavam'' as configurações
de outros projetos.

Nossa primeira acao foi forcar a execucao sequencial de forma manual
colocando os parametros de configuracao sequencial em todos os \pomf{}
dos projetos. 
}
