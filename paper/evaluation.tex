...
From these 159 unstable subjects, \Jbc{elaborate what is unstable (eg,
(1) how many could not resolve dependencies? (2) how many could not
compile sources and tests? (3) could not run tests due to incompatible
testing environment?}. Our final set consists in \numSubjs{} testable
subjects.
...
SSH server). In addition, we configured the \CodeIn{isolcpus} option
...
passed all tests and 118 had failures. \Jbc{elaborate failed subjects:
how much \% of the tests failed?}
...
\numSubjsTotal{} projects we analyzed.  However, use of parallelism
does not assure speedups on those projects.  We analyzed one sensible
factor that is critical to the effectiveness of parallelism: the
amount of tests that dominate execution time.  In the limit, if cost
...
We evaluated the dispersion of time distributions (one distribution
per project) to aswer research question RQ2.  To measure dispersion
\emph{across} projects we used Relative Standard Deviation
(RSD)~\cite{everitt-book-stats-2010}.  Note that, if we were to
analyze each project in isolation, the standard deviation of a
...
the distribution is.  Consequently, ignoring other factors that affect
performance of parallelization (e.g., CPU/IO ratio), the lower the
value of RSD the more parallelizable a test suite should be.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\textwidth]{R/relativeSD.pdf}  
  \caption{\label{fig:relativesd}Distribution of RSD ($\sigma/\mu$)
    across projects.\Fix{update (mocked) data}}
\end{figure}
...
projects with \shortg{}-running test suites as we observed\Mar{please,
  confirm} that those projects often\Mar{adjust} do not use
parallelism; including them would favor sequential execution in our
analysis.

%% We
%% focused in these two groups because given that costly test suites
%% exist, we investigated if the maintainers are aware of parallelization
%% settings.

To answer RQ3\Mar{use macro} we initially mined build files (\ie,
\pomf{}) that contain parallelization keywords, namely
\CodeIn{forkMode}, \CodeIn{forkCount}, and \CodeIn{parallel}.
According to the Maven surefire documentation\Fix{cite surefire doc},
...
cost.\Mar{quickly explain other columns}

%% summarize observations...
From the \numMedLong{} subjects, we found \pomMedLong{} \pomf{} files
where 109\Mar{use macro} of those files match one of the keywords
mentioned above.  From these 109\Mar{use macro} build files, we
...
comment, and three were commented configurations. The 106\Fix{fix
  this} build files with valid configurations are distributed across
41\Mar{use macro} projects from our sample.  \emph{From these results
  we found that $\sim$51\% of medium and long-running projects do not
  use parallel features to run test suites.}\Mar{please make it
  consistent with research question}\Mar{explain this is
...
\begin{figure}[ht!]
    ...
    projects with time-consuming tests. \Fix{explain columns}}
    \label{tab:inspection-table} 
\end{figure}

\subsubsection{Distribution of parallel modes}

\Jbc{Conclusion and relate the results to the
  RQ...........................................}
...
\begin{figure}[h!]
    ...
    identified in \Fix{M} projects\Jbc{Currently medium and partially
    the long group...}}
\end{figure}
