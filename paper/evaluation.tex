...
execution cost, issues of flakiness that hinders use of
...
\newcommand{\RQF}{How the time cost from a test suite is distributed
among the test cases?}
\newcommand{\RQB}{How often are parallelization features in
build files?}
...
common in open-source projects. The \Fix{X} research question
addresses the balance of regression tests. We want to investigate how
the time cost of a test suite is distributed among test cases. The
...
JUnit test cases in parallel using Maven Surefire) and what setting
are more often used.  The third research question addresses the impact
of parallelism on the regression tests from our sample set. For
...
and deactivated (\ie, sequential execution). The fourth research
question addresses the characteristics of regression tests from the
previous experiment.  More specifically, we want to investigate if
there is a relation between the balance of test execution (\ie, how
uniformly distributed is the execution time across tests cases) and
the usage of computational resources (\ie, if tests are mostly CPU or
IO intense) that impacts the effectiveness of parallelization.
...
\subsection{Answering research question RQF}
\label{sec:rqF}
...
\emph{medium} identified in Section~\ref{sec:rqone}. We focused on
