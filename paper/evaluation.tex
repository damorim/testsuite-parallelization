...
It is worth mentioning that we used \Jbc{custom} JUnit \Jbc{runners}
as opposed to Maven to run \Comment{configurations \emph{C*}}test
suites with different parallel configurations (see
Section~\ref{sec:modes})\Jbc{$\leftarrow$ Reviewer 1 was confused with
the notation \emph{C*} AND the original sentence was not accurate to
this paragraph}.
After carefully verifying library versions
...
implemented basic JUnit test runners (one for each \Jbc{parallel}
...
could occur) we used the \CodeIn{timeout} command~\cite{timeout-cmd}
...
Figure~\ref{tab:rq6-table} summarizes results ordered by subject's
