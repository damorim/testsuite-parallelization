...
source artifacts are publicly available (on request) for
replication\Comment{ at \Fix{create gh-pages}}.  This includes
supporting scripts (\eg, the script that test subjects and generates
the raw data), and the full list of projects. \Comment{, and a
...
\CodeIn{apache.maven-surefire} runs all unit tests in a few seconds in
...
\fbox{
\begin{minipage}{8cm}
    \textit{Answering \numRQA{}:}~\emph{We concluded that
    time-consuming regression test suites are relatively frequent in
    open-source projects given that \percentMedLongRunning{} of the
    projects are costly (\ie, \numMedLong{} projects from \medg{} and
    \longg{}).}
\end{minipage}
}
\end{center}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.5\textwidth}
      ...
      \begin{tabular}{rrrr}
        ...
        \bottomrule
      \end{tabular}
      \caption{\label{fig:sd}\Fix{adjust values}}    
    \end{subfigure}%
...
horizontal red lines), namely \Fix{0.41s} for \medg{} projects and
\Fix{0.23s} for \longg{} projects.  Note that correlation between cost
...
long-running.  This project contains only \Fix{65} tests, \Fix{62} of
...
indicates that the project \CodeIn{wildfly-swarm} was an extreme case;
we did not find projects with similar characteristics of time
monopolization.  Project \CodeIn{jenkinsci.gerrit-trigger-plugin} is
...
\longg{}-running.
\Jbc{rework this... $\rightarrow$}To reduce the number of build files
...
\CodeIn{forkCount}, and \CodeIn{parallel}.  This mining step, however,
...
\fbox{
  \begin{minipage}{8cm}
      ...
      that parallelism features are underused given that only \Fix{\%}
      of projects supports parallel execution of test suites.}
  \end{minipage}
}
\end{center}
...
To enforce sequential execution, \Luis{rework this $\rightarrow$} we 
