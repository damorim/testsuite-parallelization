...
projects that are in activity. In addition, we used Maven as a
reference due to its popularity on Java projects and to automate our
evaluation. We used the Github's Search API~\cite{githubsearch} to
fetch Java projects with Maven support according to the following
criteria: (1) the primary language must be Java\footnote{In case of
projects in multiple languages, the Github API considers the
...
trivial subjects. The fourth criteria is an approximation to find
projects with Maven support. The rationale is that if the string
\emph{mvn} exists in the \emph{readme} file, it may represents a Maven
call in the instructions on how to compile (or test) the project.
Figure~\ref{fig:subject-query} illustrates the query.
...
\caption{\label{fig:subject-query} Query to the Github API for
    ...
    2016 (or later), and \emph{readme} file contains the string
    \emph{mvn}. Output listed in descending order of stars}
\end{figure}

As of December 12th 2016, our search criteria returned 589 subjects.
