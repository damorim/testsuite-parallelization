...
execution of tests.  We observed that only bootstrapping the Maven
process suffices to discover which parallel configuration modes it
will use to run tests.  According to Maven's
...
each module discarding duplicates as to avoid inflating results. For
instance, if a project contains two modules using the same
configuration, we counted only one occurrence.
...
select dynamically which one should be used.  We automatically parsed
\Fix{X} build files and we used the same approach to classify the
parsed configurations as in our previous analysis. Initially, we
noticed that our approach was unable to infer the parallel mode in
some projects because these projects do not declare a static value in
some parameters (\eg,
\CodeIn{<parallel>\$\{parallel.type\}</parellel>}). For these projects,
it is required to inform additional parameters to enable the parallel
mode in the command line (\eg, \CodeIn{mvn test
-Dparallel.type=classesAndMethods}). \Jbc{We wouldn't have FALSE
NEGATIVES if we considered this $\rightarrow$}To handle this case, we
considered all valid values for the parameter.
