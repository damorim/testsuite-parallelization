...
3.3.9.
We used Python\Comment{ 3.4}, Bash, R and Ruby\Comment{ 2.3} to
...
\Comment{, and a \emph{Vagrantfile} to emulate our hardware and all
...
Figure~\ref{fig:rq1-boxplot} shows as boxplots distribution of cost of
...
which take nearly 11s to run, four of which takes on average 3m to run, and two of
...
cases worth special attention: \CodeIn{Chaos}, \CodeIn{Takes} and
...
class cannot be broken down with the selected configuration.
\Jbc{
On the other hand, the speedup observed in project \CodeIn{Takes} was
...
Finally, we note that parallel execution in \CodeIn{Log4J2} was
