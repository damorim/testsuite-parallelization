...
a \Fix{...describe machine...}  We focused on configuration \ForkSeq{}
\begin{wrapfigure}{r}[0pt]{0pt}%0.525\linewidth
    \centering
    \includegraphics[width=.2\textwidth]{R/scalability/scalability.pdf}
    \caption{\label{fig:scalability}Scalability.}
\end{wrapfigure}
as the goal is to evaluate the impact of spawning a growing number of
JVMs, running on different CPUs.  We selected the subject
\subjectScalability{} in this experiment as it represents the case of
a long-running test suite (see Figure~\ref{tab:speedup}) distributed
across many test classes (\Fix{X} test classes for
\subjectScalability{}).  Note that a test class is the smallest unit
that can be used to spawn a test job on a JVM.
