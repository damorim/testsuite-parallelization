...
avoid noise in our measurements, we first discarded projects
containing failing tests (note that we measure flakiness).  Then, we
sorted projects in descending order of their execution times and
selected the top ten projects from the resulting set.

In this experiment, the baseline technique is \Seq{} and the
evaluation techniques are all the configuration modes listed in
Section~\ref{sec:modes}.  We ran each of the technique for \Fix{X}
times reporting elapsed time and number of failing tests each time.
We reported average results.
