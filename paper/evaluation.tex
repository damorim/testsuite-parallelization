...
To answer \numRQD{}, we considered all \numProjectsPar{} subjects from
our benchmark that use parallelization by default in Maven to run
tests.  Section~\ref{sec:rqC-1} describes in detail our methodology to
identify these subjects. For each project, the speedup (or slowdown)
corresponds to the ratio between the elapsed time of a sequential
execution and the elapsed time of a corresponding parallel execution,
obtained as described in the script from
Figure~\ref{fig:mvn-execution}.  As usual, a ratio above 1x denotes
speedup and a ratio below 1x denotes slowdown.
...
Figure~\ref{tab:speedup} summarizes our findings by increasing order
...
observed that \Fix{falar sobre o resultado geral dos speedups -
elaborar menor e maior speedup... acho que so vale a pena discutir
quando tiver conviccao dos 2 casos}
...
projects with the parallel execution modes \ForkSeq{}, \ParClassSeqMeth{} and
...
\begin{figure}[ht!]
    ...
    \begin{tabular}{llcl}
        ...
        Medium & Apache Flink & 74 & \ForkSeq{} \\%
        Medium & BounceStorage Chaos HTTP Proxy & 1 & \ParClassSeqMeth{}\\%
        Medium & Hazelcast Jet & 7 & \ForkSeq{}\\%
        Medium & Jankotek MapDB & 1 & \ParClassParMeth{}\\%
        Medium & Javaslang & 3 & \ParClassParMeth{}\\%
        Medium & Jcabi Github & 1 & \ParClassParMeth{}\\%
        Medium & Spotify Helios & 8 & \ForkSeq{}\\%
        \bottomrule%
    \end{tabular}}
