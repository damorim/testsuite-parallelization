...
the \SeqClassParMeth{} setting, \Fix{elaborate and give example...}.
For the \ParClassSeqMeth{} setting, \Fix{elaborate and give
example...}. Finally, the \ParClassParMeth{} setting
\Fix{...elaborate...}.
...
parallelism levels. For instance, \Fix{elaborate...}.  However, some
settings may not take effect since only one test class can run in a
forked process at time. For instance, \Fix{elaborate...}. To the best
...
\begin{itemize}
\item
    \textbf{Forked classes; sequential methods (\ForkSeq).}~\Fix{todo}
\item
    \textbf{Forked classes; parallel methods (\ForkParMeth).}~\Fix{todo}
\end{itemize}

\subsubsection*{Illustration}~\Fix{elaborate}
...
\end{lstlisting}
    ...
    (\CodeIn{parallel} parameter) using two threads
    (\CodeIn{threadCount} parameter) per available core (flag
