...
Several build systems support the execution of test suites in forked
processes running a Java virtual machine (JVM).  Each spawned process
runs an individual test suite and the builder outputs the aggregated
reports. When the testing framework finishes the execution, the forked
process can be reused for the next test suite in the queue. In this
level, the number of forked processes must be proportional to the
number of available CPUs otherwise, the high degree of concurrency
may affect the execution performance.  For instance, if the underlying
machine has four CPUs available, the builder could be configured to
spawn one process on each core to favor parallelism (in contrast to
several processes competing for a single core).
...
dataraces~\Fix{cite}.  JUnit implements these modes through a set of
customized classes (e.g., \CodeIn{ParallelComputer}).

\subsubsection*{Example}~ \Fix{The list below shortly describes
the choices to control parallelism within a JVM.}
...
\caption{\label{fig:surefire} Example of Maven configuration: test
  methods run in parallel (\CodeIn{parallel} parameter) using two
  threads (\CodeIn{threadCount} parameter) per available core (flag
  \CodeIn{perCoreThreadCount}).}
\end{figure}
