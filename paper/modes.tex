...
and Ant, this amounts to spawning one JVM instance to handle a test
job and aggregating corresponding results when test jobs finish.  Note
that forking can only be combined with configuration
\SeqClassParMeth{} (see Section~\ref{sec:frameworks}) as Maven only
accepts one test class at a time per forked process.  To attenuate the
non-trivial cost of spawning new JVM processes on every test class,
Maven offers an option to reuse the JVM.
...
Maven implements this feature through its JUnit test plugin
Surefire~\cite{maven-surefire-plugin}.  With this configuration, Maven
