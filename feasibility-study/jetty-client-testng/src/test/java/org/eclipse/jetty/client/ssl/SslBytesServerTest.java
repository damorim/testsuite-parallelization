...
public class SslBytesServerTest extends SslBytesTest {
	...
	public void testHandshake() throws Exception {
		...
		AssertJUnit.assertNotNull(record);
		...
		AssertJUnit.assertNull(handshake.get(5, TimeUnit.SECONDS));

		// Check that we did not spin
		TimeUnit.MILLISECONDS.sleep(500);
		MatcherAssert.assertThat(sslFills.get(), Matchers.lessThan(1250));
		MatcherAssert.assertThat(sslFlushes.get(), Matchers.lessThan(1250));
		MatcherAssert.assertThat(httpParses.get(), Matchers.lessThan(1250));
	...
	public void testHandshakeWithResumedSessionThenClose() throws Exception {
		...
		client1.close();
		AssertJUnit.assertTrue(automaticProxyFlow.stop(5, TimeUnit.SECONDS));
		...
		AssertJUnit.assertNotNull(doneRecord);
		// Close
		client2.close();
		TLSRecord closeRecord = proxy.readFromClient();
		AssertJUnit.assertNotNull(closeRecord);
		AssertJUnit.assertEquals(TLSRecord.Type.ALERT, closeRecord.getType());
		// Flush to server Client Key Exchange + Client Done + Close in one
		// chunk
		...
		record = proxy.readFromServer();
		AssertJUnit.assertNotNull(record);
		AssertJUnit.assertEquals(TLSRecord.Type.ALERT, record.getType());
		record = proxy.readFromServer();
	...
	public void testHandshakeWithSplitBoundary() throws Exception {
		...
		// Check that we did not spin
		TimeUnit.MILLISECONDS.sleep(500);
		MatcherAssert.assertThat(sslFills.get(), Matchers.lessThan(1240));
		MatcherAssert.assertThat(sslFlushes.get(), Matchers.lessThan(1250));
		MatcherAssert.assertThat(httpParses.get(), Matchers.lessThan(1250));
		...
		// Socket close
		record = proxy.readFromClient();
		AssertJUnit.assertNull(String.valueOf(record), record);
		proxy.flushToServer(record);

		// Socket close
		record = proxy.readFromServer();
		if (record != null) {
			AssertJUnit.assertEquals(record.getType(), Type.ALERT);
	...
	public void testRequestResponse() throws Exception {
		...
		TLSRecord record = proxy.readFromClient();
		proxy.flushToServer(record);
		AssertJUnit.assertNull(request.get(5, TimeUnit.SECONDS));

		// Application data
		record = proxy.readFromServer();
		AssertJUnit.assertEquals(TLSRecord.Type.APPLICATION, record.getType());
		proxy.flushToClient(record);
		...
		AssertJUnit.assertTrue(line.startsWith("HTTP/1.1 200 "));
	...
	public void testHandshakeAndRequestOneByteAtATime() throws Exception {
		...
		AssertJUnit.assertNull(handshake.get(1, TimeUnit.SECONDS));
		...
		for (byte b : record.getBytes())
			proxy.flushToServer(5, b);
		AssertJUnit.assertNull(request.get(1, TimeUnit.SECONDS));
		...
		MatcherAssert.assertThat(sslFills.get(), Matchers.lessThan(125000));
		MatcherAssert.assertThat(sslFlushes.get(), Matchers.lessThan(1250));
		// An average of 958 httpParses is seen in standard Oracle JDK's
		// An average of 1183 httpParses is seen in OpenJDK JVMs.
		MatcherAssert.assertThat(httpParses.get(), Matchers.lessThan(125000));
	...
	public void testRequestWithCloseAlertAndShutdown() throws Exception {
		...
		// SSLSocket is limited and we cannot read the response, but we make
		// sure
	...
	public void testRequestWithCloseAlert() throws Exception {
		...
		// It is required that the other party respond with a close_notify alert
		// of its own
		// and close down the connection immediately, discarding any pending
		// writes. It is not
	...
	public void testRequestWithCloseAlertWithSplitBoundary() throws Exception {
		if (!OS.IS_LINUX) {
			...
			// TODO (react to this portion which seems to allow win/mac
			// behavior)
			// It is required that the other party respond with a close_notify
			// alert of its own
			// and close down the connection immediately, discarding any pending
			// writes. It is not
	...
	public void testRequestWithBigContentWithSplitBoundary() throws Exception {
		...
		// Use a content that is larger than the TLS record which is 2^14
		// (around 16k)
		byte[] data = new byte[128 * 1024];
		Arrays.fill(data, (byte) 'X');
		final String content = new String(data, StandardCharsets.UTF_8);
		...
		// Check that we did not spin
		TimeUnit.MILLISECONDS.sleep(500);
		MatcherAssert.assertThat(sslFills.get(), Matchers.lessThan(1200));
		MatcherAssert.assertThat(sslFlushes.get(), Matchers.lessThan(1250));
		MatcherAssert.assertThat(httpParses.get(), Matchers.lessThan(1200));
	...
	public void testRequestWithContentWithRenegotiationInMiddleOfContentWhenRenegotiationIsForbidden()
			throws Exception {
		...
		TLSRecord record = proxy.readFromClient();
		AssertJUnit.assertEquals(TLSRecord.Type.HANDSHAKE, record.getType());
		proxy.flushToServer(record);
		...
		// Trying to write more application data results in an exception since
		// the server closed
		record = proxy.readFromClient();
		proxy.flushToServer(record);
		try {
	...
	public void testRequestWithBigContentWithRenegotiationInMiddleOfContent() throws Exception {
		...
		record = proxy.readFromServer();
		AssertJUnit.assertEquals(TLSRecord.Type.CHANGE_CIPHER_SPEC, record.getType());
		proxy.flushToClient(record);
		...
		AssertJUnit.assertNull(renegotiation.get(5, TimeUnit.SECONDS));
	...
	public void testRequestWithBigContentWithRenegotiationInMiddleOfContentWithSplitBoundary() throws Exception {
		...
		System.arraycopy(bytes, chunk1.length, chunk2, 0, chunk2.length);
		proxy.flushToServer(100, chunk1);
		// Do not write the second chunk now, but merge it with content, see
		// below
	...
	public void testServerShutdownOutputClientDoesNotCloseServerCloses() throws Exception {
		...
		AssertJUnit.assertEquals(-1, client.getInputStream().read());
		...
		// The server has shutdown the output since the client sent a
		// Connection: close
		// but the client does not close, so the server must idle timeout the
		// endPoint.

		TimeUnit.MILLISECONDS.sleep(idleTimeout + idleTimeout / 2);

		AssertJUnit.assertFalse(serverEndPoint.get().isOpen());
	}
	...
	public void testPlainText() throws Exception {
		...
		// Instead of passing the Client Hello, we simulate plain text was
		// passed in
	...
	public void testRequestConcurrentWithIdleExpiration() throws Exception {
		...
		AssertJUnit.assertTrue(latch.await(idleTimeout * 2, TimeUnit.MILLISECONDS));
		...
		MatcherAssert.assertThat(((Dumpable) server.getConnectors()[0]).dump(),
				Matchers.not(Matchers.containsString("SCEP@")));
	}

	// private void assumeJavaVersionSupportsTLSRenegotiations() {
	// // Due to a security bug, TLS renegotiations were disabled in JDK
	// // 1.6.0_19-21
	...
	private SSLSocket newClient(SimpleProxy proxy) throws IOException, InterruptedException {
		...
		AssertJUnit.assertTrue(proxy.awaitClient(5, TimeUnit.SECONDS));
		return client;
	}
