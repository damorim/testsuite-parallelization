...
public class HttpRequestAbortTest extends AbstractHttpClientServerTest
{
    ...
    public void testAbortBeforeQueued() throws Exception
    {
        ...
        {
            AssertJUnit.assertSame(failure, x.getCause());
            ...
            AssertJUnit.assertEquals(1, connectionPool.getConnectionCount());
            AssertJUnit.assertEquals(0, connectionPool.getActiveConnections().size());
            AssertJUnit.assertEquals(1, connectionPool.getIdleConnections().size());
        }
    }
    ...
    public void testAbortOnQueued() throws Exception
    {
        ...
        catch (ExecutionException x)
        {
            ...
            if (aborted.get())
                AssertJUnit.assertSame(cause, x.getCause());
            AssertJUnit.assertFalse(begin.get());
        }

        HttpDestinationOverHTTP destination = (HttpDestinationOverHTTP)client.getDestination(scheme, "localhost", connector.getLocalPort());
        DuplexConnectionPool connectionPool = destination.getConnectionPool();
        AssertJUnit.assertEquals(0, connectionPool.getConnectionCount());
        AssertJUnit.assertEquals(0, connectionPool.getActiveConnections().size());
        AssertJUnit.assertEquals(0, connectionPool.getIdleConnections().size());
    }
    ...
    public void testAbortOnBegin() throws Exception
    {
        ...
        catch (ExecutionException x)
        {
            ...
            if (aborted.get())
                AssertJUnit.assertSame(cause, x.getCause());
            AssertJUnit.assertFalse(committed.await(1, TimeUnit.SECONDS));
        }
    ...
    public void testAbortLongPollAsync() throws Exception
    {
        ...
        request.send(new Response.CompleteListener()
        {
            ...
            public void onComplete(Result result)
            {
                ...
                AssertJUnit.assertSame(cause, result.getFailure());
                latch.countDown();
            }
        });
