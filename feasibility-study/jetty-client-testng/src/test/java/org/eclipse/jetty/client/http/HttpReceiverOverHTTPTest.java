...
public class HttpReceiverOverHTTPTest {
	...
	protected HttpExchange newExchange() {
		...
		HttpExchange exchange = new HttpExchange(destination, request,
				Collections.<Response.ResponseListener>singletonList(listener));
		boolean associated = connection.getHttpChannel().associate(exchange);
		AssertJUnit.assertTrue(associated);
	...
	public void test_Receive_NoResponseContent() throws Exception {
		...
		AssertJUnit.assertEquals("OK", response.getReason());
		AssertJUnit.assertSame(HttpVersion.HTTP_1_1, response.getVersion());
		HttpFields headers = response.getHeaders();
		AssertJUnit.assertNotNull(headers);
		AssertJUnit.assertEquals(1, headers.size());
		AssertJUnit.assertEquals("0", headers.get(HttpHeader.CONTENT_LENGTH));
	}
	...
	public void test_Receive_ResponseContent() throws Exception {
		...
		AssertJUnit.assertNotNull(headers);
		AssertJUnit.assertEquals(1, headers.size());
		AssertJUnit.assertEquals(String.valueOf(content.length()), headers.get(HttpHeader.CONTENT_LENGTH));
		String received = listener.getContentAsString(StandardCharsets.UTF_8);
		AssertJUnit.assertEquals(content, received);
	}
	...
	public void test_Receive_ResponseContent_EarlyEOF() throws Exception {
		...
		try {
			...
			AssertJUnit.assertTrue(e.getCause() instanceof EOFException);
		}
	}
	...
	public void test_Receive_ResponseContent_IdleTimeout() throws Exception {
		...
		try {
			...
			AssertJUnit.assertTrue(e.getCause() instanceof TimeoutException);
		}
	}
	...
	public void test_Receive_BadResponse() throws Exception {
		...
		try {
			...
			AssertJUnit.assertTrue(e.getCause() instanceof HttpResponseException);
		}
	}
