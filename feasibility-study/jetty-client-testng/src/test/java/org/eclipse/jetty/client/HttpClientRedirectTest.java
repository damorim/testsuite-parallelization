...
public class HttpClientRedirectTest extends AbstractHttpClientServerTest
{
    ...
    public void test_303() throws Exception
    {
        ...
        AssertJUnit.assertFalse(response.getHeaders().containsKey(HttpHeader.LOCATION.asString()));
    }
    ...
    public void test_301_WithWrongMethod() throws Exception
    {
        ...
        catch (ExecutionException x)
        {
            ...
            AssertJUnit.assertNotNull(response);
            AssertJUnit.assertEquals(301, response.getStatus());
            AssertJUnit.assertTrue(response.getHeaders().containsKey(HttpHeader.LOCATION.asString()));
        }
    }
    ...
    public void testMaxRedirections() throws Exception
    {
        ...
        catch (ExecutionException x)
        {
            ...
            Response response = xx.getResponse();
            AssertJUnit.assertNotNull(response);
            AssertJUnit.assertEquals(302, response.getStatus());
            AssertJUnit.assertTrue(response.getHeaders().containsKey(HttpHeader.LOCATION.asString()));
        }
    }
    ...
    public void testDontFollowRedirects() throws Exception
    {
        ...
        Response response = client.newRequest("localhost", connector.getLocalPort())
                ...
                .send();
        AssertJUnit.assertNotNull(response);
        AssertJUnit.assertEquals(303, response.getStatus());
        AssertJUnit.assertTrue(response.getHeaders().containsKey(HttpHeader.LOCATION.asString()));
    }
    ...
    public void testRedirectFailed() throws Exception
    {
        ...
        {
            MatcherAssert.assertThat(x.getCause(), Matchers.instanceOf(UnresolvedAddressException.class));
        }
    }
    ...
    public void testHttpRedirector() throws Exception
    {
        ...
        AssertJUnit.assertEquals(303, response1.getStatus());
        AssertJUnit.assertTrue(redirector.isRedirect(response1));
        ...
        AssertJUnit.assertEquals(302, response2.getStatus());
        AssertJUnit.assertTrue(redirector.isRedirect(response2));

        final CountDownLatch latch = new CountDownLatch(1);
        redirector.redirect(request2, response2, r ->
        {
            Response response3 = r.getResponse();
            AssertJUnit.assertEquals(200, response3.getStatus());
            AssertJUnit.assertFalse(redirector.isRedirect(response3));
            latch.countDown();
        });
        AssertJUnit.assertTrue(latch.await(5, TimeUnit.SECONDS));
    }
    ...
    public void testRedirectWithCorruptedBody() throws Exception
    {
        ...
        AssertJUnit.assertEquals(200, response.getStatus());
        AssertJUnit.assertArrayEquals(bytes, response.getContent());
    }
