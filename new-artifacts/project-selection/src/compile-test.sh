#!/bin/bash
#
# Invokes maven to compile and test projects in a given directory
#
...
for PROJECT in `ls "$INPUT_DIR"`; do
    ...
    PROJECT_OUTPUT="$(pwd)/$OUTPUT_DIR/$PROJECT/$TS"
    ...
    timeout -s SIGKILL $TIMEOUT mvn -X verify -fae $MAVEN_SKIP 2>&1 \
